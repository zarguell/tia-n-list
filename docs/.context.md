...
**Simplified Workflow:**
1. **RSS Ingestion** → Get fresh articles from last 24 hours
2. **Quality Filtering** → Filter for content length and relevance
3. **Memory Deduplication** → Exclude articles recently used in digests
4. **Simple Synthesis** → Single LLM call for digest generation
5. **Hugo Output** → Create blog post with basic metadata
...
# Configure digest behavior (optional)
export SIMPLE_DIGEST_MAX_ARTICLES=15          # Default: 10
export SIMPLE_DIGEST_MAX_CONTENT_LENGTH=2000   # Default: 1500
export SIMPLE_DIGEST_MIN_CONTENT_LENGTH=300    # Default: 200
export SIMPLE_DIGEST_MAX_REFERENCES=20        # Default: 15
export SIMPLE_DIGEST_MEMORY_DAYS_BACK=14      # Default: 7
export SIMPLE_DIGEST_MEMORY_CLEANUP_DAYS=60   # Default: 30
export SIMPLE_DIGEST_MAX_TOKENS=4000          # Default: 3000

# Run simple digest script
python scripts/generate_simple_digest.py
```

## 🎭 **PROFESSIONAL PERSONA MODULE** (LEGACY)
...
python scripts/test_simple_modules.py
...
## 🎯 **SIMPLIFIED DIGEST APPROACH - FURTHER REDUCTION**
...
1. **SimpleDigestGenerator**: Streamlined RSS→digest workflow
...
4. **Simple Workflow**: Fresh articles → Memory filter → LLM synthesis → Hugo output
...
**Refactoring Achievements (November 2025):**
- **Reduced cyclomatic complexity** from 17 to <9 per method
- **Extracted theme detection logic** into dedicated method
- **Extracted datetime normalization** for better maintainability
- **Added configuration constants** for all hard-coded limits
...
**Error Handling:**
- Structured exception handling with specific error messages
- Logging at appropriate levels (INFO, ERROR, DEBUG)
- Graceful degradation when components fail
- Better debugging capabilities with detailed logs

**Code Organization:**
- Single responsibility principle applied to extracted methods
- Clear separation of concerns (datetime, themes, formatting)
