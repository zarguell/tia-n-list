...
The system now supports multiple LLM providers (Gemini, OpenRouter, OpenAI) with automatic fallback and enhanced configuration:
...
- `LLM_MAX_TOKENS=1500` - General purpose token limit
- `LLM_MAX_TOKENS_BLOG=3000` - Blog generation token limit (increased from 1500)
...
- Supports private deployments, custom providers, and alternative services

**Configuration Examples:**

```bash
# Use OpenRouter with enhanced token limits
...
export LLM_MAX_TOKENS_BLOG=4000  # Higher limit for comprehensive briefings

# Use OpenAI with custom endpoint
export LLM_PROVIDER=openai
export OPENAI_API_KEY=your_key_here
export OPENAI_BASE_URL=https://custom-provider.com/v1
export LLM_MAX_TOKENS_BLOG=5000  # Maximum content generation

# Use standard OpenAI with enhanced defaults
export LLM_PROVIDER=openai
export OPENAI_API_KEY=your_key_here
# Automatic max_tokens/max_completion_tokens compatibility included
...
- **Automatic parameter detection**: Handles both `max_tokens` and `max_completion_tokens`
- **GPT-5-nano support**: Automatic fallback to `max_completion_tokens` when required
- **Error-transparent retry**: Seamless parameter switching without manual intervention
- **Universal compatibility**: Works with all OpenAI-compatible models
...
**Enhanced Benefits:**
- **100% more content**: Blog generation increased from 1500 → 3000 tokens by default
- **Provider flexibility**: Use any OpenAI-compatible API endpoint
- **Zero breaking changes**: All existing configurations continue to work unchanged
- **Intelligent compatibility**: Automatic handling of model-specific API requirements
